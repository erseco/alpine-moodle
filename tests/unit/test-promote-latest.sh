...
trap 'rm -f "$log"' EXIT
export log
export remote_tags=$'hash refs/tags/v5.2.3\nhash refs/tags/v5.2.9\nhash refs/tags/v5.2.10\nhash refs/tags/v5.3.0-beta\nhash refs/tags/v5.3.0\nhash refs/tags/v4.5.14'
git() { [[ "${fail_git:-false}" == false ]] || return 1; printf '%s\n' "$remote_tags"; }
docker() { printf '%s\n' "$*" >> "$log"; [[ "${fail_docker:-false}" == false ]]; }
export -f git docker
digest=sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa
[[ $(bash scripts/release-policy.sh --newest) == v5.2.10 ]]
bash scripts/promote-latest.sh v5.2.10 "$digest" example/moodle
[[ $(wc -l < "$log") -eq 2 ]]
...
for version in v5.2.9 v5.2.3 v4.5.14 v5.3.0-beta v5.3.0-rc1 v5.3.0 main; do
...
must_fail v5.2.11 'sha256:invalid' example/moodle
must_fail v5.2.11 "$digest" 'example/moodle;id'
must_fail 'v5.2.11;id' "$digest" example/moodle
export fail_git=true
must_fail v5.2.11 "$digest" example/moodle
export fail_git=false remote_tags=''
bash scripts/promote-latest.sh v5.2.11 "$digest" example/moodle
[[ $(wc -l < "$log") -eq 2 ]]
export remote_tags='hash refs/tags/v5.2.11' fail_docker=true
must_fail v5.2.11 "$digest" example/moodle
